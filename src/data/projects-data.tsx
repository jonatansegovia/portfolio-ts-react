export const projects_english = [
  {
    imgLink:
      'https://github.com/jonatansegovia/webservice/blob/main/imgs/start-screen.png?raw=true',
    appLink: 'https://pf-web-service.vercel.app/',
    title: 'E-commerce Services for Everybody | HENRY',
    subheader: 'NOVEMBER - 2021',
    altText: 'e-commerce-app miniature',
    shortDescription: 'E-commerce for regular services',
    longDescription:
      'E-commerce as a final project. Developed together with a team of 8 people. We use Trello and GitHub. This app points to offer sales of services. Some of the features implemented: Backend with Express and relational table models with PostgreSQL / Sequelize. Front-end part with React / Redux, Registration, Login, Payments (MercadoPago, Paypal), Material UI, Admin panel, User Panel, chat, filters for services, dark mode, and responsiveness for the whole app.',
  },
  {
    imgLink:
      'https://media-exp1.licdn.com/dms/image/sync/C4D27AQEmZjOnRbnWoQ/articleshare-shrink_1280_800/0/1649719078285?e=2147483647&v=beta&t=NtVi3CMuH_JlZFPpZn_Y569RBawsbkXkT4VvsvWO2X8',
    appLink: 'https://countries-app-roan.vercel.app/',
    title: 'Countries App | HENRY',
    subheader: 'OCTOBER - 2021',
    altText: 'countries-app miniature',
    shortDescription:
      'Filter Countries of the World, Alphabetically, Bigger and Smaller, etc.',
    longDescription:
      'Development of a SPA(Single Page Application) using React and Redux for the FrontEnd, styled with CSS modules. It consumes data from an external API (restcountries) through a BackEnd developed in NodeJs using Express and a relational DataBase in Postgress using Sequealize as the ORM. Some features of the project are: Different types of Filters, a search bar, and a controlled form to create your own recipe.',
  },
  {
    imgLink:
      'https://www.nintenderos.com/wp-content/uploads/2021/09/pokemon-go-pikachu-arcoiris.jpg',
    appLink: 'https://jonatansegovia.github.io/PokemonSearcher/',
    title: `Pokemon Searcher`,
    subheader: 'MARCH - 2021',
    altText: 'pokemon-app miniature',
    shortDescription: 'My first page using HTML5, CSS3 in combination witch JS',
    longDescription: `In this app you can search for a Pokemon in the search bar, and see the output data with an image from this. I implemented vanilla JS for consuming the Pokemon API.`,
  },
  {
    imgLink:
      'https://github.com/jonatansegovia/narutofan-spage/blob/master/img/Naruto-Backgrounds-1.jpg?raw=true',
    appLink: 'https://jonatansegovia.github.io/narutofan-spage/index.html',
    title: `Naruto's Fan Page`,
    subheader: 'DECEMBER - 2020',
    altText: 'naruto-app miniature',
    shortDescription: 'My first page using HTML5 and CSS3',
    longDescription: `Basic page for giving my first steps in the world of HTML and CSS. This tries to reach the hearth of Naruto's fan, and compile in videos linked from YouTube, the best moments of this amazing anime.`,
  },
  {
    imgLink:
      'https://github.com/jonatansegovia/portfolio-ts-react/blob/main/public/portofolio.PNG',
    appLink: 'https://portfolio-ts-react-js.herokuapp.com/',
    title: `Naruto's Fan Page`,
    subheader: 'DECEMBER - 2020',
    altText: 'naruto-app miniature',
    shortDescription: 'My first page using HTML5 and CSS3',
    longDescription: `Basic page for giving my first steps in the world of HTML and CSS. This tries to reach the hearth of Naruto's fan, and compile in videos linked from YouTube, the best moments of this amazing anime.`,
  },
];

export const projects_spanish = [
  {
    imgLink:
      'https://github.com/jonatansegovia/webservice/blob/main/imgs/start-screen.png?raw=true',
    appLink: 'https://pf-web-service.vercel.app/',
    title: 'E-commerce Servicios Para Todos | HENRY',
    subheader: 'NOVIEMBRE - 2021',
    altText: 'e-commerce-app miniature',
    shortDescription: 'E-commerce para servicios de uso cotidiano',
    longDescription:
      'E-commerce de ventas y adquisición de servicios. Para el front-end utilizamos principalmente React, Redux y Material UI. Para el back-end usamos principalmente Express, Sequelize y PostgreSQL. Algunas de las caracteristicas principales: Login/Register(incluye google), Panel de Usuario(favs, servicios, órdenes), Panel de Admin(estadísticas, servicios, usuarios), Carrito, Checkout(Mercadopago/Paypal), Chat(live/offiline socket.io). El proyecto fue llevado a cabo por un equipo de 8 integrantes.',
  },
  {
    imgLink:
      'https://media-exp1.licdn.com/dms/image/sync/C4D27AQEmZjOnRbnWoQ/articleshare-shrink_1280_800/0/1649719078285?e=2147483647&v=beta&t=NtVi3CMuH_JlZFPpZn_Y569RBawsbkXkT4VvsvWO2X8',
    appLink: 'https://countries-app-roan.vercel.app/',
    title: 'Countries App | HENRY',
    subheader: 'OCTUBRE - 2021',
    altText: 'countries-app miniature',
    shortDescription:
      'Filtra países del mundo, Alfabéticamente, Más grandes o más chicos, etc.',
    longDescription:
      'Desarrollo de una SPA (Single Page Apllication) usando React and Redux para el Front-End. La SPA consume datos de una API externa (https://restcountries.com/v2/all), comunicándose con un Back-End desarrollado con Express, Node y PostgreSQL, usando Sequelize como ORM. Las características principales de la SPA son: filtrado, búsqueda por nombre, paginado, creación de actividades turísticas.',
  },
  {
    imgLink:
      'https://www.nintenderos.com/wp-content/uploads/2021/09/pokemon-go-pikachu-arcoiris.jpg',
    appLink: 'https://jonatansegovia.github.io/PokemonSearcher/',
    title: `Buscador de Pokemons`,
    subheader: 'MARZO - 2021',
    altText: 'pokemon-app miniature',
    shortDescription:
      'Mi primer página usando HTML5, CSS3 en combinación con JS',
    longDescription: `En esta app podés buscar por un Pokemon en la barra de búsqueda, y ver los detalles de esta búsqueda con su correspondiente imagen. Implementé vanilla JS para consumir la Pokemon API.`,
  },
  {
    imgLink:
      'https://github.com/jonatansegovia/narutofan-spage/blob/master/img/Naruto-Backgrounds-1.jpg?raw=true',
    appLink: 'https://jonatansegovia.github.io/narutofan-spage/index.html',
    title: `Naruto's Fan Page`,
    subheader: 'DICIEMBRE - 2020',
    altText: 'naruto-app miniature',
    shortDescription: 'Mi primer página usando solo HTML5 and CSS3',
    longDescription: `Página básica para dar mis primeros pasos en el mundo de la programación. Esta intenta alcanzar el corazón de los fans de Naruto, y compila en videos linkeados desde YouTube, los mejores momentos de este.`,
  },
];
